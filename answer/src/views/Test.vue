<template>
	<div style="overflow-y: scroll;">
		<Bg2>
			...
			<!-- <ej-question :subjet="subjectlist[activecode]"></ej-question> -->
		</Bg2>
...
<script>
	...
	// 	GetDailyTest
	// } from '@/request/api.js'
	// import EjQuestion from '@/components/EjQuestion.vue'
	import Bg2 from "@/components/Bg2.vue"
	import Head from "@/components/dailytest/Head.vue"
	export default {
		data() {
			return {
				subjectlist: '',
		...
		methods: {
			...
			last() {
				...
				window.console.log(this.activecode)
			},
			next() {
				if (this.activecode == this.subjectlist.length - 1) {
					alert("已经是最后一题了")
				} else {
					this.activecode += 1
				}
